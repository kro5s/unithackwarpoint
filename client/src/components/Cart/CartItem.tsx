import React, {useState} from 'react';

interface Props {
    name: string;
    content: string;
    img: string;
    price: string;
}

const CartItem: React.FC<Props> = ({ name, content, price, img }) => {
    const [counter, setCounter] = useState(1)

    return (
        <div className="p-6 bg-dark-secondary shadow rounded flex">
            <img src="https://placehold.co/600x400" alt="" className="size-[140px] rounded object-cover"/>
            <div className="max-w-[30%] ml-6 mr-auto">
                <div className="my-3"><span className="font-bold text-xl">{name}</span></div>
                <p className="line-clamp-3">{content}</p>
            </div>
            <div className="flex flex-col items-end">
                <div className="mb-auto"><span className="text-2xl font-bold">{price}</span></div>
                <div className="flex items-center gap-x-6">
                    <div className="flex gap-x-3 items-center border border-white rounded py-1 px-2 font-semibold">
                        <span>-</span>
                        <span>{counter}</span>
                        <span>+</span>
                    </div>
                    <svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M3 18C2.45 18 1.97917 17.8042 1.5875 17.4125C1.19583 17.0208 1 16.55 1 16V3H0V1H5V0H11V1H16V3H15V16C15 16.55 14.8042 17.0208 14.4125 17.4125C14.0208 17.8042 13.55 18 13 18H3ZM13 3H3V16H13V3ZM5 14H7V5H5V14ZM9 14H11V5H9V14Z"
                            fill="white"/>
                    </svg>
                </div>
            </div>
        </div>
    );
};

export default CartItem;